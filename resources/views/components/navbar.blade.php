        ...
        <div class="ml-auto flex items-center mr-3">
            @if (Session::has('profile_picture'))
                <a href="{{ route('showProfile', Session::get('user_id')) }}">
                    <img src="{{ Session::has('profile_picture') ? asset(Session::get('profile_picture')) : asset('assets/pfp.jpg') }}"
            ...
            @if (Session::has('user_id'))
                <a href="{{ route('logout') }}"
                    class="px-3 py-2 bg-red-500 text-white hover:bg-red-700 rounded shadow-md text-sm font-bold ml-3"><i
